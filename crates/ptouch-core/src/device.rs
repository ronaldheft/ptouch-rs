...
bitflags! {
    ...
    pub struct DeviceFlags: u32 {
        ...
        /// Device accepts caller-rendered raster data at twice its normal
        /// feed-axis resolution when high quality is selected.
...
mod tests {
    ...
    fn p710bt_supports_native_feed_high_resolution() {
        let device = find_device(0x04f9, 0x20af).unwrap();
        assert!(device.flags.contains(DeviceFlags::FEED_HIRES));
        assert!(!device.flags.contains(DeviceFlags::LEGACY_HIRES));
    }
