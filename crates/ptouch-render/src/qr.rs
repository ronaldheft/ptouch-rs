...
fn source_modules(
    ...
    // qrcodegen asserts on a wrong data length. Validate using the existing
    // encoder's capacity table rather than catching a panic from user input.
    let capacity = qrcode::bits::Bits::new(qrcode::Version::Normal(i16::from(source.version)))
        .max_len(error_correction.into())
        .map_err(|error| RenderError::Layout(format!("QR source version is invalid: {error}")))?
        / 8;
    if data_codewords.len() != capacity {
        return Err(RenderError::Layout(format!(
    ...
    let code = SourceQrCode::encode_codewords(
        Version::new(source.version),
        error_correction.into(),
        &data_codewords,
        Some(Mask::new(source.mask_pattern)),
    );
...
mod tests {
    ...
    #[test]
    fn wrong_source_capacity_returns_an_error_without_panicking() {
        for version in 1..=40 {
            for level in [
                ...
                let source = QrSource {
                    data_codewords_base64: "AA==".into(),
                    version,
                    mask_pattern: 0,
                };
                assert!(
                    source_modules(&source, level)
                        .unwrap_err()
                        .to_string()
